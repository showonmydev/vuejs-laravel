      ...
      <div class="col-xs-12 right-tabs-con">
        ...
        <div class="tab-content">					
          ...
          <div class="tab-pane control_div" id="control">
          	<div class="controls_tabs text-12">
						<div class="clearfix margin-btm controls_tabs_top">
							<p><strong v-if="tableFullInfo"><span v-bind:title="tableFullInfo.name">{{tableFullInfo.name | trimDeviceName}}</span></strong>
							</p>
							...
							<div class="controls_btns" v-if="tableFullInfo">
								...
								<button type="button" class="btn " v-bind:class="{ 'btn off-btn-color': tableFullInfo.power == 'off' }" @click="UpdateMetaValue('off','power')">Standby</button>
							</div>
						</div>
          	...
          	<div class="filex-row">
						<ul class="controls_tabs_list">
							...
							<li>
								...
								<div class="controls_btns">
									<button type="button" @click="OnInputSource()" class="btn orange-btn" v-if="tableFullInfo">{{tableFullInfo.inputSource}}<i class="fa fa-angle-right"></i></button>
							...
							<li>
								...
								<div class="controls_btns" v-if="tableFullInfo">
									<button type="button" @click="onPictureMode(pictureMode, 'pictureMode')" class="btn orange-btn">{{tableFullInfo.pictureMode}} <i class="fa fa-angle-right"></i></button>
								</div>
							</li>
						</ul>
						<ul class="controls_tabs_list">
						<li>
								...
								<div class="controls_btns" v-if="tableFullInfo">
									...
									<button type="button" class="btn " v-bind:class="{ 'off-btn-color': tableFullInfo.avmute == 'off' }" @click="UpdateMetaValue('off','avmute')">Off</button>
								</div>
							</li>
						<li>
								...
								<div class="controls_btns" v-if="tableFullInfo">
									...
									<button type="button" class="btn " v-bind:class="{ 'off-btn-color': tableFullInfo.audioMute == 'off' }" @click="UpdateMetaValue('off','audioMute')">Off</button>
								</div>
							</li>
							<li>
								...
								<div class="controls_btns" v-if="tableFullInfo">
									...
									<button type="button" class="btn " v-bind:class="{ 'off-btn-color': tableFullInfo.screenBlank == 'off' }" @click="UpdateMetaValue('off','screenBlank')">Off</button>
								</div>
							</li>													
						</ul>
          </div>
						<div class="clearfix col-sm-12" style="max-width: 420px;">
          	...
          	<div class="clearfix software_upgrade">
          		...
          		<div class="clearfix margin-btm software_upgrade_top">
					...
					<div class="pull-right" v-if="$store.state.Setting.StoreVmVarDeviceUpdt.updateReq == 'Yes'">
						...
						<span class="Upgrade_version" v-if="$store.state.NeedUpdate.get_app">{{$store.state.NeedUpdate.get_app.get_need_update_version.version_name}}<i @click="upgradeNow(1,'','',$store.state.NeedUpdate)" :title="$store.state.NeedUpdate.get_app.description" v-tippy="{ placement : 'bottom',  theme: 'tool', duration: 500, arrow: true, size: 'large' }" aria-hidden="true" class="img-icon-benq-1 icon-10"></i></span>
					</div>
					<div class="pull-right" v-if="$store.state.Setting.StoreVmVarDeviceUpdt.updateReq == 'No'">
						...
						<span class="Upgrade_version" v-if="tableFullInfo">{{tableFullInfo.version}} <i @click="upgradeNow(2,'','',tableFullInfo)" :title="tableFullInfo.description" v-tippy="{ placement : 'bottom',  theme: 'tool', duration: 500, arrow: true, size: 'large' }" aria-hidden="true" class="img-icon-benq-1 icon-10"></i></span> 
					</div>
					<div class="">
				...
				<table class="table table-responsive">				
					...
					<tbody>
						<tr v-for="(list, key) in $store.state.DisplaySoftwareList" class="TH" v-bind:key="key" v-bind:data-identify="list.id" @click="selectThisApp(list)">							
							<td v-if="list.get_app.get_app_version">{{list.get_app.name}}<a class="hover-div" href="javascript:void(0)" @click="upgradeNow(2,'edit',list)"><i class="pull-right tooltipDiv webIcon appInfoTag" aria-hidden="true" :title="list.get_app.name" v-tippy="{ placement : 'bottom',  theme: 'tool', duration: 500, arrow: true, size: 'large' }"></i></a></td>
          ...
          <section class="modal-body text-12">
            <slot name="body" v-if="tableFullInfo">
              <div class="text-center input_source">
							<div class="col-sm-12"><button type="button" class="btn bg-fff-brd" v-bind:class="{ 'orange-btn': tableFullInfo.inputSource == 'HDMI-1' }" @click="UpdateMetaValue('HDMI-1','inputSource')">HDMI-1 <i class="fa fa-angle-right"></i></button></div>
							<div class="col-sm-12"><button type="button" class="btn bg-fff-brd" v-bind:class="{ 'orange-btn': tableFullInfo.inputSource == 'HDMI-2' }" @click="UpdateMetaValue('HDMI-2','inputSource')">HDMI-2 <i class="fa fa-angle-right"></i></button></div>
							<div class="col-sm-12"><button type="button" class="btn bg-fff-brd" v-bind:class="{ 'orange-btn': tableFullInfo.inputSource == 'AV' }" @click="UpdateMetaValue('AV','inputSource')">AV<i class="fa fa-angle-right"></i></button></div>
							<div class="col-sm-12"><button type="button" class="btn bg-fff-brd" v-bind:class="{ 'orange-btn': tableFullInfo.inputSource == 'VGA-1' }" @click="UpdateMetaValue('VGA-1','inputSource')">VGA-1 <i class="fa fa-angle-right"></i></button></div>
							<div class="col-sm-12"><button type="button" class="btn bg-fff-brd" v-bind:class="{ 'orange-btn': tableFullInfo.inputSource == 'VGA-2' }" @click="UpdateMetaValue('VGA-2','inputSource')">VGA-2 <i class="fa fa-angle-right"></i></button></div>
            </div>
						</slot>
          ...
          <section class="modal-body">
            <slot name="body" v-if="tableFullInfo">
              ...
              <div class="col-sm-12"><button type="button" class="btn" v-bind:class="{ 'orange-btn': tableFullInfo.pictureMode == 'Picture Mode' }" @click="UpdateMetaValue('Picture Mode','pictureMode')">Picture Mode <i class="fa fa-angle-right"></i></button></div>
							<div class="col-sm-12"><button type="button" class="btn" v-bind:class="{ 'orange-btn': tableFullInfo.pictureMode == 'Normal Mode' }" @click="UpdateMetaValue('Normal Mode','normalMode')">Normal Mode <i class="fa fa-angle-right"></i></button></div>
							</div>
            </slot>
						<p class="text-center">Command may take a while to apply. </p>
          </section>
          ...
          <footer class="modal-footer">
              <slot name="footer">
                <button type="button" class="btn-success btn" @click="RenameDevice('Rename')">Ok</button>
...
<!-- 			<ul class="second_modal" v-if="settings.updateDeviceBlock">
          ...
          <footer class="modal-footer text-center pull-right">
              <slot name="footer">
...
<p>Please help confirm the operation.</p>
          ...
          <section class="modal-body device-Version-Info">
            <div class="software_upgrade_table">
						<h5>Available on server:</h5>
						<table class="table table-responsive border-all">				
							<thead>
								<tr>
									<th></th>
									<th @click="sort('app_name')">Name <span class="sort-icon pull-right" v-show="displayAppColumnSorting.isForAppName != 'NOT'"><a v-if="isAscending" @click="sort('app_name')">▼</a>
                  <a v-else @click="sort('app_name')">▲</a></span> </th>
									<th @click="sort('version')">Version <span class="sort-icon pull-right" v-show="displayAppColumnSorting.isForAppversion != 'NOT'"><a v-if="isAscending" @click="sort('version')">▼</a>
                  <a v-else @click="sort('version')">▲</a></span></th>
									<th @click="sort('date_modified')">Date modified <span class="sort-icon pull-right" v-show="displayAppColumnSorting.isForDateModify != 'NOT'"><a v-if="isAscending" @click="sort('date_modified')">▼</a>
                  <a v-else @click="sort('date_modified')">▲</a></span></th>
									<th @click="sort('size')">Size <span class="sort-icon pull-right" v-show="displayAppColumnSorting.isSize != 'NOT'"><a v-if="isAscending" @click="sort('size')">▼</a>
                  <a v-else @click="sort('size')">▲</a></span></th>
									<!-- <th>APP Update</th> -->
								</tr>
							</thead>
							<tbody>
								<tr v-for="(list, key) in tableData" v-bind:key="key">
									...
									<td><div class="checkbox radio-dott1" @click="updateTableCheck('A'+key+1,list)"><label><input type="checkbox" style="display:none" v-model="forCheckBox['A'+key+1]" value=""><span class="cr"><i class="cr-icon radio-dott"></i></span></label></div></td>
									<!-- <td>{{list.app_name}}<a class="hover-div" href="javascript:void(0)"><i class="img-icon-benq-1 icon-10" aria-hidden="true"></i> <div class="hover-open-div"><div class="div_flex"><ul><li><span><strong>APP Name</strong></span> <span>{{list.app_name}}</span></li> <li><span><strong>Description</strong></span> <span>{{list.description}}</span></li> <li><span><strong>Version</strong></span> <span>{{list.version}}</span></li></ul> </div> </div></a></td> -->
									<td>{{list.app_name}}</td>
									<td>{{list.version}}</td>
		...
		<button type="button" v-if="checkIfDeviceCheck" disabled class="btn-default btn pull-right" @click="UpdateDeviceInSearch(forCheckBox)">Install</button>
          ...
          <footer class="modal-footer">
              <slot name="footer">
								<button type="button" class="btn-default btn" @click="InstallDeviceViaPopUP()">Save</button>
...
export default {
  ...
  data () {
    return {
			displayAppColumnSorting : {'isForAppName':true, 'isForAppversion':'NOT',  'isForDateModify':'NOT', 'isSize':'NOT'},
    ...
    tableFullInfo: function () {
      ...
      return this.$store.state.Setting.DisplayIndividualData
		...
		tableData:function() { 
         ...
         if(this.$store.state.Setting.ListAllApplication.length > 0){
  ...
  mounted () {
	...
	methods : {
		sort:function(s) {
			 if(s == "app_name"){
					this.displayAppColumnSorting.isForAppName = true;
					this.displayAppColumnSorting.isForAppversion = 'NOT';
					this.displayAppColumnSorting.isForDateModify = 'NOT';
					this.displayAppColumnSorting.isSize = 'NOT';
			 }
			 if(s == "version"){
				  this.displayAppColumnSorting.isForAppName = 'NOT';
					this.displayAppColumnSorting.isForAppversion = true;
					this.displayAppColumnSorting.isForDateModify = 'NOT';
					this.displayAppColumnSorting.isSize = 'NOT';
			 }
			 if(s == "date_modified"){
				  this.displayAppColumnSorting.isForAppName = 'NOT';
					this.displayAppColumnSorting.isForAppversion = 'NOT';
					this.displayAppColumnSorting.isForDateModify = true;
					this.displayAppColumnSorting.isSize = 'NOT';
			 }
			 if(s == "size"){
				  this.displayAppColumnSorting.isForAppName = 'NOT';
					this.displayAppColumnSorting.isForAppversion = 'NOT';
					this.displayAppColumnSorting.isForDateModify = 'NOT';
					this.displayAppColumnSorting.isSize = true;
			 }
...
.right-sidebar-toggleForTab:hover .icon-1{
    background-position: 46% 50%;
		background-color: #ec6c00 !important;
		-webkit-transition: 0s !important;
    transition: 0s !important;
}
li.active > .right-sidebar-toggleForTab .icon-1 {
    background-position: 46% 50%;
}
.icon-2 {
		background-position: 35.4% 50%;
}
.right-sidebar-toggleForTab:hover .icon-2{
    background-position: 30% 50%;
		background-color: #ec6c00 !important;
		-webkit-transition: 0s !important;
    transition: 0s !important;
}
li.active > .right-sidebar-toggleForTab .icon-2 {
    background-position: 30% 50%;
}
.icon-3 {
		background-position: 67.5% 50%;    
}
.right-sidebar-toggleForTab:hover .icon-3{
    background-position: 62.2% 50%;
		background-color: #ec6c00 !important;
		-webkit-transition: 0s !important;
    transition: 0s !important;
}
li.active > .right-sidebar-toggleForTab .icon-3 {
    background-position: 62.2% 50%;
}
.icon-5 {
    background-position: 174% -25%;
}
.icon-6 {
    background-position: 172.8% 0%;
    display: inline-block;
...
.icon-8 {
    background-position: 2.5% 9%;
    width: 30px !important;
}
.icon-9 {
    background-position: -22.7% 9%;
    width: 30px !important;
}
.icon-10 {
    background-position: 96.5% -60%;
...
span.icon-right.img-icon-benq-1.icon-5.icon-5-riverse {
...
.radio-dott1 .cr .cr-icon,
.radio .cr .cr-icon .radio-dott {
position: absolute;
font-size: .8em;
line-height: 0;
top: 13%;
...
.border-all thead tr th:first-child {
border-right: 1px solid #cecece;
}
.border-all thead tr th {
border-bottom: 1px solid #cecece !important;
}
.border-all tbody tr td {
border-top: none;
}
.radio-dott1 label, .radio label {
padding-left: 9px;
}
.border-all tbody tr:last-child {
height: auto;
}
...
.add-new-device-padding .radio-dott1 label {
display: flex;
align-items: center;
padding-bottom: 0;
}
